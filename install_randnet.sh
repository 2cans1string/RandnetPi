...
info "Generating dnsmasq Randnet config (domains -> $ETH0_IP)..."
mkdir -p /etc/dnsmasq.d
# Generate the config at install time so the address= targets resolve to the
# Pi's real eth0 IP (where Tomcat/Squid are reachable), NOT 127.0.0.1.
cat > /etc/dnsmasq.d/randnet.conf <<EOF
# Generated by install_randnet.sh — Randnet domains resolve to this Pi's eth0 IP
address=/randnet.ne.jp/${ETH0_IP}
address=/login.randnet.ne.jp/${ETH0_IP}
address=/www.randnet.ne.jp/${ETH0_IP}
address=/member.randnet.ne.jp/${ETH0_IP}
address=/mail.randnet.ne.jp/${ETH0_IP}
address=/news.randnet.ne.jp/${ETH0_IP}
address=/service.randnet.ne.jp/${ETH0_IP}
address=/peach.randnet.ne.jp/${ETH0_IP}
address=/keiba.randnet.ne.jp/${ETH0_IP}
address=/randnetdd.co.jp/${ETH0_IP}
address=/www.randnetdd.co.jp/${ETH0_IP}
EOF
info "dnsmasq Randnet config written to /etc/dnsmasq.d/randnet.conf"
